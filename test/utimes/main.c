...
static int __test_futimesat(const char *file_path) {
    ...
    times[0] = actime;
    times[1] = modtime;
    ret = futimesat(dirfd, file_name, times);
    if (ret < 0) {
...
static int __test_utimensat(const char *file_path) {
    ...
    times[0] = actime;
    times[1] = modtime;
    ret = utimensat(dirfd, file_name, times, 0);
    if (ret < 0) {
        THROW_ERROR("failed to futimesat file with dirfd");
    }
    close(dirfd);
...
typedef int(*test_utime_func_t)(const char *);

static int test_utime_framework(test_utime_func_t fn) {
...
static int test_utime() {
    return test_utime_framework(__test_utime);
}

static int test_utimes() {
    return test_utime_framework(__test_utimes);
}

static int test_futimesat() {
    return test_utime_framework(__test_futimesat);
}

static int test_futimesat_nullpath() {
    return test_utime_framework(__test_futimesat_nullpath);
}

static int test_utimensat() {
    return test_utime_framework(__test_utimensat);
}
