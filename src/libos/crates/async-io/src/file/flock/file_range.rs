...
impl FileRange {
    /// Create the range through C flock and opened file offset and file size
    pub fn from_flock_with_file_metadata(
        lock: &flock_c,
        offset: usize,
        size: usize,
    ) -> Result<Self> {
        let start = {
            ...
            match whence {
                ...
                RangeLockWhence::SEEK_CUR => (offset as off_t)
                    .checked_add(lock.l_start)
                    .ok_or_else(|| errno!(EOVERFLOW, "start overflow"))?,
                RangeLockWhence::SEEK_END => (size as off_t)
                    .checked_add(lock.l_start)
                    .ok_or_else(|| errno!(EOVERFLOW, "start overflow"))?,
            }
        };
