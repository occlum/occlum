...
pub fn do_fcntl(fd: FileDesc, cmd: &mut FcntlCmd) -> Result<isize> {
    ...
    let ret = match cmd {
        ...
        FcntlCmd::GetLk(lock_mut_c) => {
            let file = file_table.get(fd)?;
            let lock_type = FlockType::from_u16(lock_mut_c.l_type)?;
            if FlockType::F_UNLCK == lock_type {
                return_errno!(EINVAL, "invalid flock type for getlk");
            }
            let mut lock = FlockBuilder::new()
                .owner(file_table.id())
                .type_(lock_type)
                .range(FlockRange::from_c_flock_and_file(&lock_mut_c, &file)?)
                .build()?;
            file.test_advisory_lock(&mut lock)?;
            trace!("getlk returns: {:?}", lock);
            (*lock_mut_c).copy_from_safe(&lock);
            0
        }
        FcntlCmd::SetLk(lock_c) => {
            let file = file_table.get(fd)?;
            let lock = FlockBuilder::new()
                .owner(file_table.id())
                .type_(FlockType::from_u16(lock_c.l_type)?)
                .range(FlockRange::from_c_flock_and_file(&lock_c, &file)?)
                .is_nonblocking(true)
                .build()?;
