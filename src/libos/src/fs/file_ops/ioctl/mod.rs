...
use self::builtin::*;
pub use self::builtin::{GetIfConf, GetWinSize, IfConf, SetWinSize, TcGets, TcSets};
pub use self::commands::{GetIfReqWithRawCmd, GetReadBufLen, IoctlCmd, SetNonBlocking};
...
impl<'a> IoctlRawCmd<'a> {
    ...
    pub fn validate_arg_and_ret_vals(&self, ret: i32) -> Result<()> {
        match self {
            IoctlRawCmd::TIOCGWINSZ(winsize_ref) => {
            ...
            IoctlRawCmd::FIONREAD(nread_ref) => {
                if (**nread_ref < 0) {
