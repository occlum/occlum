...
pub fn do_socket(domain: c_int, type_and_flags: c_int, protocol: c_int) -> Result<isize> {
    // Check arguments
    ...
    let file_ref = Arc::new(socket_file);
...
pub fn do_bind(fd: c_int, addr: *const libc::sockaddr, addr_len: libc::socklen_t) -> Result<isize> {
    ...
    let socket_file = file_ref.as_uring_socket()?;
...
pub fn do_accept4(
    ...
    // Do accept
    let nonblocking = flags.contains(SocketFlags::SOCK_NONBLOCK);
    let accepted_socket = socket_file.accept(nonblocking)?;
...
pub fn do_recvfrom(
    ...
    let (bytes_recv, addr_recv) = socket_file.recvfrom(&mut buf, flags)?;
...
pub fn do_sendmsg(fd: c_int, msg_ptr: *const libc::msghdr, flags: c_int) -> Result<isize> {
    debug!(
...
pub fn do_recvmsg(fd: c_int, msg_mut_ptr: *mut libc::msghdr, flags: c_int) -> Result<isize> {
    debug!(
    ...
    let (bytes_recv, recv_addr, msg_flags, msg_controllen) =
        socket_file.recvmsg(&mut bufs[..], flags, control)?;
...
pub fn do_sendmmsg(
    ...
    let file_ref = current!().file(fd as FileDesc)?;
    let socket_file = file_ref.as_uring_socket()?; // .as_socket_file()
