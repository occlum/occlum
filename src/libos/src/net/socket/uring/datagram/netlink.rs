...
impl<A: Addr, R: Runtime> NetlinkSocket<A, R> {
    ...
    pub fn sendmsg(
        ...
        if control != None {
            return_errno!(EINVAL, "netlink socket sendmsg don't support msg_control");
        }
        let res = if addr.is_some() {
            self.sender.sendmsg(bufs, addr.unwrap(), flags, control)
        } else {
            ...
            self.sender.sendmsg(bufs, &peer_addr, flags, control)
        };
