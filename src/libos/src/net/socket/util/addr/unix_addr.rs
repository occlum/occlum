...
impl UnixAddr {
    ...
    pub fn from_c(c_addr: &libc::sockaddr_un, c_len: usize) -> Result<Self> {
        ...
        if c_addr.sun_family != libc::AF_UNIX as libc::sa_family_t {
            return_errno!(Errno::EINVAL, "an unix address is expected");
        }
        if c_len < std::mem::size_of::<libc::sa_family_t>() {
            return_errno!(Errno::EINVAL, "the length of the address is too small");
        } else if c_len > std::mem::size_of::<libc::sockaddr_un>() {
            return_errno!(Errno::EINVAL, "the length of the address is too large");
        }
        ...
        if path_len == 1 {
            ...
            return_errno!(Errno::EINVAL, "the pathname must not be empty");
        }
        ...
        if c_addr.sun_path[0] != 0 {
            ...
            if last_char != 0 {
                return_errno!(Errno::EINVAL, "the pathname is not null-terminated");
            }
            ...
            let pathname = {
                ...
                if path_cstr.to_bytes_with_nul().len() > MAX_PATH_LEN {
                    return_errno!(Errno::EINVAL, "no null in the address");
                }
                path_cstr
                    .to_str()
                    .map_err(|_| errno!(Errno::EINVAL, "path is not UTF8"))?
                    .to_string()
            };
    ...
    pub fn from_c_storage(c_addr: &libc::sockaddr_storage, c_addr_len: usize) -> Result<Self> {
        if (c_addr_len) > std::mem::size_of::<libc::sockaddr_storage>() {
            return_errno!(Errno::EINVAL, "address length is too large");
        }
    ...
    pub fn path_str(&self) -> Result<&str> {
        match self {
            ...
            Self::Unnamed => return_errno!(Errno::EINVAL, "can't get path name for unnamed socket"),
        }
    }
