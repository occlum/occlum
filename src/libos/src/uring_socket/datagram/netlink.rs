...
use crate::uring_socket::socket::MsgFlags;
use crate::uring_socket::socket::{NetlinkFamily, Type};
...
impl<A: Addr, R: Runtime> NetlinkSocket<A, R> {
    ...
    pub fn readv(&self, bufs: &mut [&mut [u8]]) -> Result<usize> {
        self.recvmsg(bufs, RecvFlags::empty()).map(|(ret, ..)| ret)
    }
    ...
    pub fn recvmsg(
        ...
        self.receiver.recvmsg(bufs, flags, None)
    }
    ...
    pub fn writev(&self, bufs: &[&[u8]]) -> Result<usize> {
        self.sendmsg(bufs, None, SendFlags::empty())
    }

    pub fn sendmsg(&self, bufs: &[&[u8]], addr: Option<&A>, flags: SendFlags) -> Result<usize> {
