...
{
    echo ""
    echo "*** Doing iperf3 with ${CPUS} CPUs, ${STREMS} client streams in parallel ***"
    echo "*** with read/write buffer length ${BUF_LEN} for ${TEST_TIME} seconds. ***"

    pushd occlum_server
    occlum run --cpus ${CPUS} /bin/iperf3 -s -p 6777 -1 1>/dev/null &
    popd

    sleep 3

    pushd occlum_client
    occlum run --cpus ${CPUS} /bin/iperf3 -c 127.0.0.1 -p 6777 -f Mbits \
